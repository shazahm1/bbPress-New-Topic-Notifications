...
class ja_bbPress_Topic_Notifications {
	...
	function __construct() {
		...
		add_action( 'bbp_new_topic',        array( $this, 'send_notification' ),    100, 4   );
	...
	function email_template_textarea() {
		...
		$shortcodes = '[topic-title], [topic-content], [topic-excerpt], [topic-url], [topic-author], [topic-author-url], [topic-whois-author-ip]';
	 ...
	 */
	function send_notification( $topic_id, $forum_id, $anonymous_data, $topic_author ) {
		...
		$topic_whois_author = sprintf( __( 'Whois  : http://whois.arin.net/rest/ip/%s', 'ja_bbp_notifications' ), bbp_current_author_ip() );
		...
		$find           = array( '[topic-title]', '[topic-content]', '[topic-excerpt]', '[topic-author]', '[topic-url]', '[topic-author-url]' , '[topic-whois-author-ip]' );
		$replace        = array( $topic_title, $topic_content, $topic_excerpt, $topic_author, $topic_url, $topic_author_url, $topic_whois_author );
		$email_body     = str_replace( $find, $replace, $email_template );
		$email_subject  = '[' . $blogname . '] ' . __( 'Topic:', 'ja_bbp_notifications' ) . ' "' . $topic_title . '"';

		$wp_email = get_bloginfo( 'admin_email', 'display' );
		...
		if ( 'Anonymous' == $topic_author ) {
			...
			$from = "From: \"$topic_author\" <$wp_email>";
		...
		if ( !empty( $recipients ) ) {
			...
			foreach ( $recipients as $recipient ) {
				@wp_mail( $recipient, $email_subject, $email_body, $message_headers );
			}
		}
	}
